...
pub fn SystemStats() -> Element {
    ...
    let mem_label = format!("{:.1}% ({:.1} GB / {:.1} GB)", 
        mem_percent, 
        memory_used as f64 / 1_073_741_824.0, 
        memory_total as f64 / 1_073_741_824.0
    );

    let (net_rx, net_tx) = networks.iter().fold((0, 0), |acc, n| (acc.0 + n.received, acc.1 + n.transmitted));
    let net_label = format!("RX: {:.1} MB | TX: {:.1} MB", 
